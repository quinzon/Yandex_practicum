...
async def test_get_oauth_client_success(oauth_client, oauth_service):
    with patch.object(
            oauth_service.oauth_client,
            'create_client',
            return_value=AsyncMock()
    ) as mock_create_client:
        client = await oauth_service.get_oauth_client('provider')
        assert client is not None
        mock_create_client.assert_called_once_with('provider')
...
async def test_authorize_access_token(oauth_service, mock_request):
    ...
    with patch.object(
        ...
        token_response = await oauth_service.authorize_access_token('provider', mock_request)
...
async def test_invalid_access_token(oauth_service, mock_request):
    with patch.object(
        ...
        with pytest.raises(Exception, match="Invalid token"):
            await oauth_service.authorize_access_token('provider', mock_request)
        mock_authorize.assert_called_once_with('provider', mock_request)
...
async def test_get_user_info_failure(oauth_service):
    ...
    with patch.object(
        ...
        with pytest.raises(Exception, match="Failed to get user info"):
            await oauth_service.get_user_info('provider', mock_token)
        mock_get_client.assert_called_once_with('provider')
